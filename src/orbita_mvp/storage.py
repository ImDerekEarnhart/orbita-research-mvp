    ...
    def delete_case(self, case_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if owned_claim_ids:
                ...
                delete(f"DELETE FROM claim_supersessions WHERE older_claim_id IN ({placeholders}) OR newer_claim_id IN ({placeholders})", tuple(owned_claim_ids) * 2)
                delete(f"DELETE FROM reexamination_queue WHERE claim_id IN ({placeholders})", tuple(owned_claim_ids))
                delete(f"DELETE FROM claim_checks WHERE claim_id IN ({placeholders})", tuple(owned_claim_ids))
                delete(f"DELETE FROM mvp_claim_keys WHERE claim_id IN ({placeholders})", tuple(owned_claim_ids))
